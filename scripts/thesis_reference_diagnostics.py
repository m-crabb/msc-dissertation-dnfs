...
Example: python -m scripts.thesis_reference_diagnostics --out /tmp/reference-review
...
from experiments.constrained_soft_02.analysis import (
    composition_marginal_overlay_8x8 as soft,
)
...
def fingerprint(path):
    ...
    return {"path": str(path.relative_to(REPO_ROOT)), "sha256": digest}
...
def evaluate(chains, target, inputs, out):
    ...
    energies = [
        -target.base_log_prob(chain) / (2 * target.sigma * target.d) for chain in chains
    ]
    print(
        f"{out.stem}: {len(chains)} chains, {len(reference)} frames; split uncertainty",
        flush=True,
    )
    uncertainty = reference_standard_error(chains, target.D, chain_energies=energies)
    print(f"{out.stem}: iid floor", flush=True)
    floor = sampling_floor_from_reference(
        reference, target.D, 5000, reference_energy=torch.cat(energies)
    )
    if target.D == 8:
        tv_floor = {
            name: soft.reference_tv_floor(
                reference, lambda x, w, fn=fn: fn(target, x, w)
            )
            for name, fn in (
                ("energy", soft.energy_pmf),
                ("composition", soft.composition_pmf),
            )
        }
    else:
        ...
        tv_floor = {
            name: baseline.reference_tv_floor(reference, len(chains), fn)
            for name, fn in (("energy", energy_pmf), ("magnetisation", magnet_pmf))
        }
    result = dict(
        lattice_edge=target.D,
        sigma=target.sigma,
        chain_lengths=[len(chain) for chain in chains],
        reference_uncertainty=uncertainty,
        iid_floor=floor,
        marginal_tv_iid_floor=tv_floor,
        n_draws=5000,
        n_replicates=200,
        n_splits=64,
        seed=0,
    ...
    print(
        f"{out.stem}: "
        f"{json.dumps(dict(uncertainty=uncertainty, floor=floor, tv=tv_floor))}",
        flush=True,
    )
    ...
    if args.hard_marginals:
        ...
        for edge in (8, 16):
            ...
            result = dict(
                ...
                floor_method="independent reference frames "
                "with replacement vs full pool",
                inputs=[fingerprint(path) for path in inputs],
            )
            out.write_text(json.dumps(result, indent=2, allow_nan=False) + "\n")
            print(result["marginal_tv_iid_floor"], flush=True)
        return
    for label, sigma in (("s010", 0.1), ("sc", SIGMA_C)):
        path = REPO_ROOT / f"results/01_baseline/wolff_ref_d10_sigma{sigma:g}.pt"
        pool = torch.load(path, weights_only=True)
        chains = list(
            pool["samples"].float().reshape(-1, pool["n_chains"], 100).unbind(1)
        )
        evaluate(
            chains,
            IsingTarget(D=10, sigma=sigma),
            [path],
            args.out / f"baseline_{label}.json",
        )
        for composition in (0.25, 0.375, 0.5):
            paths = sorted(
                (REPO_ROOT / "results/mchammer_vcsgc").glob(
            ...
            for path, chain, potential in zip(paths, chains, potentials):
                assert torch.allclose(
                    -target.base_log_prob(chain),
                    torch.from_numpy(np.load(potential)).float(),
                    atol=1e-3,
                ), path
            evaluate(
                chains,
