...
def load_family(run_dirs: list[Path]) -> dict:
    ...
    n_eval = config["eval"].get("n_eval_samples_training") or config["eval"]["n_eval_samples"]
...
def main() -> None:
    ...
    parser.add_argument("--patch_runs", required=True, nargs="+", type=Path,
                        help="two-hole patch head R=2 sigma_c run dirs (3 seeds)")
    parser.add_argument("--band_runs", required=True, nargs="+", type=Path,
    ...
    ax.text(ramp_end / 1e3 * 0.5, 1.035, "$\\sigma$ curriculum ramp",
            fontsize=FONT_SIZE_ANNOTATION, color=MUTED, ha="center")
    seed_band(ax, band["steps"] / 1e3, band["ess_fractions"], light_hue,
              "masked-attention band, one sweep")
    seed_band(ax, patch["steps"] / 1e3, patch["ess_fractions"], dark_hue,
    ...
    for name, family in (("patch R=2", patch), ("masked-attention", band)):
        ...
        print(f"{name}: last in-training eval ESS fraction per seed "
