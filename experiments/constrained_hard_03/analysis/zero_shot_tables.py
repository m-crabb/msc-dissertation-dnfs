...
D256_TEMPLATE = (
    "H2_d256_c50_s220_letf_thp2_100k_curr_b512_ne128_cv2_w3_seed{seed}_20260826-d256-sc"
)
D64_TEMPLATE = "H2_d64_c50_s220_letf_thp_50k_curr_w2_seed{seed}_20260825-hard-w2-d64"
...
def load_rows(results_dir, template, seeds=SEEDS):
    ...
    per_seed = []
    for seed in seeds:
        path = results_dir / template.format(seed=seed) / "zero_shot_transfer.json"
        if not path.is_file():
            return None
...
def cell(seed_rows):
    ...
    ratio = np.mean(
        [r["ess_fraction"] / max(r["ess_fraction_predicted"], 1e-12) for r in seed_rows]
    )
...
def temperature_table(d256):
    ...
    for key in keys:
        ...
        lines.append(
            f"        ${stop_time:.3f}$ & ${coupling:.4f}$ & {cell(seed_rows)} \\\\"
        )
    return "\n".join(lines)
