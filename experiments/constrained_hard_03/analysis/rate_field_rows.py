...
def row_for(run_dirs, tail):
    ...
    for run_dir in run_dirs:
        ...
        seed = cfg["train"]["seed"]
        if seed in seeds:
            raise ValueError(f"duplicate seed {seed}: select one completed run per seed")
        seeds.add(seed)
        ...
        if len(log) != tail:
            raise ValueError(f"need {tail} rate diagnostics in {run_dir}, found {len(log)}")
        ...
        if not np.allclose(log["sigma_current"], cfg["ising"]["sigma"], rtol=0, atol=1e-10):
            raise ValueError(f"tail spans a different target coupling: {run_dir}")
        if cfg["ctmc"]["use_matching_step"] and not np.isfinite(
...
def main():
    ...
    selection.add_argument("--cell", action="append", nargs="+", metavar="LABEL DIR",
    ...
    parser.add_argument("--tail", type=int, help="evaluations per seed (default: manifest tail or 10)")
    ...
    if args.manifest:
        ...
        cells = [[label, *(repo / path for path in paths)]
