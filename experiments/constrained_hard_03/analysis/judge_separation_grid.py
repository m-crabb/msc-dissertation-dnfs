...
ARMS = ["iv", "ivb", "mab", "fib", "fatt", "fimo2", "fmoatt"]
...
def main():
    ...
    for arm in ["ivb", "mab", "fib", "fatt", "fimo2"]:
        run = glob.glob(f"{ROOT}/H2_d64_c50_s223_letf_{arm}_50k_curr_seed42_20260823-*")[0]
        wm = wall_minutes(run, mtimes)
        print(f"  {arm:6s} raw {ess(run):.3f}  ema {ess(run,'eval_ema'):.3f}  "
              f"inner {inner_seconds(run)/60:5.1f} min  wall {wm if wm is None else f'{wm:.0f}'} min")
