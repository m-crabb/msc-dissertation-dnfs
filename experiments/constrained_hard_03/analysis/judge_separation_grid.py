...
EF_BANDS = {
    "fimo2ef": ("fimo2", 0.78, 0.765, 0.750),
    "mabef": ("mab", 0.80, 0.785, 0.769),
}
# Archived comparators (same recipe, earlier runs) -- raw eval ESS/N.
D64_REF = {
    "MA (ma, 0707)": 0.7806,
    "interval (iv, 0708)": 0.6463,
    "fmo2 (0814)": 0.745,
    "fab8 (0813)": None,
}
D16_REF_DIRS = {
    "ma": "letf_ma_10k_seed*20260708*",
...
def learned_gain(run):
    ...
    sd = torch.load(
        os.path.join(run, "checkpoints", "final.pt"),
        map_location="cpu",
        weights_only=False,
    )
...
def band_d16(vals):
    ...
    if len(ok) >= 2 and ok[1] >= 0.955:
        return "PARITY"
    if len(ok) >= 2 and ok[1] >= 0.94:
        return "MEANINGFUL"
...
def main():
    ...
    for sigma in ("s223", "s010"):
        ...
        for arm in ARMS:
            runs = sorted(
                glob.glob(f"{ROOT}/H2_d16_c50_{sigma}_letf_{arm}_10k_seed*_20260823-*")
            )
    ...
    for arm in ["ivb", "mab", "fib", "fatt", "fimo2", "fimo2ef", "mabef"]:
        run = glob.glob(
            f"{ROOT}/H2_d64_c50_s223_letf_{arm}_50k_curr_seed42_20260823-*"
        )[0]
        ...
        print(
            f"  {arm:7s} raw {ess(run):.3f}  ema {ess(run, 'eval_ema'):.3f}  "
            f"inner {inner_seconds(run) / 60:5.1f} min  wall {wm if wm is None else f'{wm:.0f}'} min{extra}"
        )
    for name, pat in {
        "MA 0707": "letf_ma_50k_curr_seed42_20260707*",
        "fmo2 0814": "letf_fmo2_50k_curr_seed42_20260814-fmo2-d64",
        "fab8 0813": "letf_fab8_50k_curr_seed42_20260813*",
    }.items():
        hits = glob.glob(f"{ROOT}/H2_d64_c50_s223_{pat}")
        if not hits:
            print(f"  ref {name:10s} (not archived locally)")
            continue
        run = hits[0]
        e, ee = ess(run), ess(run, "eval_ema")
        print(
            f"  ref {name:10s} raw {e if e is None else f'{e:.3f}'}  ema {ee if ee is None else f'{ee:.3f}'}  "
