...
def measure(reference: np.ndarray, side: int, sigma: float, seed: int) -> dict:
    ...
    for name, base in build_bases(reference, side, n_up):
        ...
        corr = (np.corrcoef(log_rho_ref, log_eta_ref)[0, 1]
                if var_log_eta > 1e-12 else None)
        ...
        delta_c_se = np.sqrt(drive_ref.var(ddof=1) / len(drive_ref)
        ...
        rows[name] = dict(
            nn_base=nn_base, nn_base_se=nn_base_se,
            var_log_eta=var_log_eta, corr=corr,
            var_drive=var_drive, ratio=var_drive / var_uniform,
            delta_c=delta_c, delta_c_se=delta_c_se,
            delta_c_energy=delta_c_energy, delta_c_eta=delta_c_eta,
            two_cov=2 * cov, trade_margin=(2 * cov / var_log_eta
                                           if var_log_eta > 1e-12 else None),
        )
    return dict(d=d, side=side, sigma=sigma, n_up=n_up, n_reference=len(reference),
                nn_target=nn_target, nn_target_se=nn_target_se,
                var_log_rho=log_rho_ref.var(), rows=rows)
...
def print_block(res: dict) -> None:
    ...
    head = (f"{'base':<12}|{'nn_base':>18}|{'Var[log eta]':>16}|{'corr':>14}|"
    ...
    for name, row in res["rows"].items():
        print(f"    {name:<12} full {row['delta_c']:7.2f} +/- {row['delta_c_se']:.2f}"
              f" = energy {row['delta_c_energy']:7.2f} - eta {row['delta_c_eta']:6.2f}"
              f"   (doc {DOC_TABLE[(res['d'], name)][5]})")
    for name, row in res["rows"].items():
        if row["corr"] is None:
            continue
        print(f"  variance-trade [{name}]: Var[log eta] {row['var_log_eta']:.1f} "
...
def print_downstream(results: dict) -> None:
    ...
    for d, res in results.items():
        ...
        for name, row in res["rows"].items():
            ...
            print(f"{d:>5}|{name:<26}|{row['nn_base']:>9.4f}|{req:>12.4f}|"
...
def main() -> None:
    ...
    d256_ref = load_d256()
    print(f"d64  reference: {d64_ref.shape} from {len(d64_meta['chains'])} chains, "
