...
def main(argv=None):
    ...
    parser.add_argument(
        "--cells", nargs="+", required=True, help="one run-dir glob per temperature"
    )
    ...
    for ax, pattern, T in zip(axes, args.cells, args.temperatures):
        ...
        runs = sorted(
            r for r in glob.glob(pattern) if os.path.exists(f"{r}/eval/samples.pt")
        )
        for run, hue in zip(runs, parameter_ramp(SAMPLER_HUE, max(len(runs), 2))):
            ax.hist(
                au_fraction(torch.load(f"{run}/eval/samples.pt")).numpy(),
                bins=bins,
                density=True,
                histtype="step",
                color=hue,
                lw=1.1,
                label="sampler, one seed" if run == runs[0] else None,
            )
        if not runs:
            ax.text(
                0.5,
        ...
        ax.set_xlabel("Au concentration", fontsize=FONT_SIZE_LABEL)
        ax.set_xlim(0.1, 0.8)
        style_axes(ax)
