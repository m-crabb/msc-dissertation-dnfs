...
for run in sorted(glob.glob(root)):
    ...
    c = cfg["ising"]["target_composition"]
    T_final = 1 / (2 * K_B * cfg["curriculum"]["stages"][-1]["sigma"])
    beta = 1 / (K_B * T_final)
    mask = n_au == round(c * d)
    e_ref = beta * E[mask]
    p = torch.softmax(-e_ref, 0)
    df = pd.read_csv(run + "/training_log.csv")
    ess = df.dropna(subset=["ess"]).set_index("step")["ess"]
    m = json.load(open(run + "/eval/metrics.json"))
    s = torch.load(run + "/eval/samples.pt").double()
    lw = torch.load(run + "/eval/log_weights.pt").double()
