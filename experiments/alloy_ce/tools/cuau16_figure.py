...
from discrete_flow_sampler.diagnostics.figure_style import (
    BAND_ALPHA, FONT_SIZE_ANNOTATION, FONT_SIZE_LABEL, FULL_WIDTH_IN, MUTED, NEURAL_COMPARATOR_HUE,
    REFERENCE_FILL, REFERENCE_INK, SAMPLER_HUE, SAVEFIG_DPI, point_errorbars, style_axes, use_house_style)
from discrete_flow_sampler.targets.cluster_expansion import BinaryExpansionSpec
from experiments.alloy_ce.tools.fcc_render import JMOL, conventional_cell, draw_structure, rectangular_tiling
...
def exact_marginal(T):
    ...
    return torch.zeros(D + 1, dtype=torch.float64).index_add_(0, N_AU, log_p.exp()).numpy()


def exact_slice_free_energy(T):
    """-log Z_n / (beta d) for n = 0..16, eV/site."""
    beta = 1.0 / (K_B * T)
    return np.array([-torch.logsumexp(-beta * ENERGY[N_AU == n], 0).item() / beta / D for n in range(D + 1)])
...
def landed(patterns):
    return sorted(r for p in patterns for r in glob.glob(p) if os.path.exists(f"{r}/eval/log_weights.pt"))


def panel_letter(ax, letter):
    ax.text(-0.02, 1.06, f"({letter})", transform=ax.transAxes, fontsize=FONT_SIZE_LABEL, color=REFERENCE_INK,
            ha="right", va="bottom")


def draw_structures(ax_l12, ax_l10):
    spec = BinaryExpansionSpec.from_json("data/ce/cuau_fcc_4x4x4.json")   # any fcc supercell gives the same cube
...
def draw_histogram(ax, runs, T):
    ax.bar(COMPOSITIONS, exact_marginal(T), width=1 / D, color=REFERENCE_FILL, alpha=0.45, zorder=1,
           label="exact", linewidth=0)
    masses = np.array([np.bincount(au_count(torch.load(f"{r}/eval/samples.pt")).numpy(), minlength=D + 1)
                       / 5000 for r in runs]) if runs else np.zeros((0, D + 1))
    if len(runs):
        ax.fill_between(COMPOSITIONS, masses.min(0), masses.max(0), step="mid", color=SAMPLER_HUE,
                        alpha=BAND_ALPHA, linewidth=0, zorder=2)
        ax.step(COMPOSITIONS, masses.mean(0), where="mid", color=SAMPLER_HUE, lw=1.4, zorder=3,
                label=f"free cell, raw draws (mean, band = min-max over {len(runs)} seeds)")
    ax.set_title(f"$T = {T}$ K", fontsize=FONT_SIZE_LABEL, pad=3)
    ax.set_xlim(0.03, 0.85); ax.set_xticks([0.25, 0.5, 0.75]); ax.set_xlabel("$c_\\mathrm{Au}$", labelpad=1)
    ax.tick_params(labelsize=FONT_SIZE_ANNOTATION); style_axes(ax)
...
def slice_estimates(runs, beta, split_by_slice):
    ...
    by_c = {}
    for run in runs:
        counts, log_w = au_count(torch.load(f"{run}/eval/samples.pt")), torch.load(f"{run}/eval/log_weights.pt")
        slices = sorted(set(counts.tolist())) if split_by_slice else [round(counts.double().mean().item())]
        for n in slices:
...
def draw_free_energy(ax_curve, ax_resid, specialists, amortised, T):
    ...
    ax_curve.plot(COMPOSITIONS, exact, "-", color=REFERENCE_INK, lw=1.2, zorder=2, label="exact (all $2^{16}$ states)")
    ...
    for n, name, dx, ha in ((4, "Cu$_3$Au", -5, "right"), (8, "CuAu", 6, "left")):
        ax_curve.annotate(name, (n / D, exact[n]), xytext=(dx, 0), textcoords="offset points",
                          ha=ha, va="center", fontsize=FONT_SIZE_ANNOTATION, color=MUTED)
    series = ((slice_estimates(specialists, beta, False), SAMPLER_HUE, "o", -DODGE, "specialist cells (one per $c$)"),
              (slice_estimates(amortised, beta, True), NEURAL_COMPARATOR_HUE, "s", +DODGE, "amortised cell (one checkpoint)"))
    for by_c, hue, marker, dodge, label in series:
        cs = np.array(sorted(by_c)); values = np.array([by_c[c] for c in cs])          # (n_c, n_seeds)
        ax_curve.plot(cs + dodge, values.mean(1), marker, color=hue, ms=4.5, zorder=4, label=label,
                      markeredgecolor="white", markeredgewidth=0.6)
        residual = values - exact[(cs * D).round().astype(int)][:, None]
        spread = np.stack([residual.mean(1) - residual.min(1), residual.max(1) - residual.mean(1)])
        point_errorbars(ax_resid, cs + dodge, residual.mean(1), spread, hue, None, marker=marker)
    ax_curve.set_xlim(-0.03, 1.03); ax_curve.set_xticks([0, 0.25, 0.5, 0.75, 1.0])
    ax_curve.set_xlabel("$c_\\mathrm{Au}$", labelpad=1); ax_curve.set_ylabel("$F(c)$ (meV/site)", labelpad=2)
    ax_curve.set_title(f"canonical free energy, $T = {T:.0f}$ K", fontsize=FONT_SIZE_LABEL, pad=3)
    ax_curve.legend(frameon=False, fontsize=FONT_SIZE_ANNOTATION - 1, loc="upper center", bbox_to_anchor=(0.56, 1.0),
                    handlelength=1.6, borderaxespad=0.2, labelspacing=0.4)
    ax_resid.axhline(0, color=REFERENCE_INK, lw=0.8, zorder=1)
    ax_resid.set_xlim(0.2, 0.55); ax_resid.set_xticks([0.25, 0.375, 0.5]); ax_resid.set_xticklabels(["0.25", "0.375", "0.5"])
    ax_resid.set_ylim(-0.2, 0.45); ax_resid.set_yticks([0, 0.2, 0.4])
    ax_resid.set_xlabel("$c_\\mathrm{Au}$", labelpad=1); ax_resid.set_ylabel("$F_\\mathrm{IS} - F_\\mathrm{exact}$ (meV/site)", labelpad=2)
...
def main(argv=None):
    ...
    parser.add_argument("--free", nargs="+", required=True, help="one glob per temperature")
    ...
    grid = GridSpec(2, 4, figure=fig, width_ratios=[0.95, 1, 1, 1], height_ratios=[1, 1.15],
                    wspace=0.55, hspace=0.65, left=0.01, right=0.99, top=0.90, bottom=0.10)
    ...
    fig.legend(handles[::-1], labels[::-1], loc="upper center", bbox_to_anchor=(0.62, 1.0), ncol=2, frameon=False,
               fontsize=FONT_SIZE_ANNOTATION, handlelength=1.6, columnspacing=1.5)
    histogram_axes[0].set_ylabel("probability mass", labelpad=2)
    draw_free_energy(ax_curve, ax_resid, landed(args.specialists), landed(args.amortised), args.fc_temperature)
    for ax, letter in zip((ax_l12, ax_l10, *histogram_axes, ax_curve, ax_resid), "abcdefg"):
        panel_letter(ax, letter)
    fig.savefig(args.out, dpi=SAVEFIG_DPI, bbox_inches="tight"); print("wrote", args.out)
