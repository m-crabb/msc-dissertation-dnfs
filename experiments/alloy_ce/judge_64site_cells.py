...
def stage_loss_over_static(run, df, spec, beta_of_sigma, n_sites, n_plus):
    ...
    stages = cfg["curriculum"]["stages"]; n_steps = cfg["train"]["n_steps"]
    ends = [s["start_step"] for s in stages[1:]] + [n_steps]
    slice_states = random_slice_states(4096, n_sites, n_plus, torch.Generator().manual_seed(0)).double()
...
def main(pattern):
    ...
    for run in sorted(p for g in pattern.split() for p in glob.glob(g)):
        ...
        for sub in ("eval", "eval_ema"):
            ...
            row.update({f"E_mean_{sub}": e_site.mean().item(), f"E_min_{sub}": e_site.min().item(),
                        f"E_is_{sub}": (w * e_site).sum().item(), f"swaps_mean_{sub}": swaps.mean().item(),
    ...
    print()
    print(table[["E_mean_eval_ema", "E_is_eval_ema", "swaps_mean_eval_ema", "swaps_is_eval_ema"]].round(2).to_string())
    print()
