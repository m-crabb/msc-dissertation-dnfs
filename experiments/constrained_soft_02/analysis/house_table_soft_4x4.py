...
def sampling_floor(target, ref_states, ref_probs, seed=0):
    ...
    for _ in range(N_BOOTSTRAP):
        ...
        replicates.append(
            errors(target, ref_states[idx], uniform, ref_states, ref_probs)
        )
...
def score_run(run_dir, target, ref_states, ref_probs, per_forward, composition=None):
    ...
    c = (x > 0).float().mean(1)
    mean_c = (w * c).sum()
    exact_f = -torch.logsumexp(target.log_prob(ref_states), 0) / (
        2 * target.sigma * D_SITES
    )
    return {
        "ESS": ess,
        **errors(target, x, w, ref_states, ref_probs),
        "FLOPes": per_effective_sample(
            neural_sampling_flops_per_sample(per_forward, n_euler, D_SITES), ess
        ),
        "std_c": ((w * (c - mean_c) ** 2).sum()).sqrt().item(),
        "dF_site": (
            free_energy_lb_estimate(log_w, target.sigma, D_SITES) - exact_f
        ).item(),
    }
...
def score_family(
    label, run_dirs, target, all_states, ref_probs, floor, exact_std_c, composition=None
):
    ...
    per_seed = {
        d.name: score_run(d, target, all_states, ref_probs, per_forward, composition)
        for d in run_dirs
    }
    summary = {
    ...
    print(
        "   mean+-SD: "
        + f"ESS {m['ESS'][0]:.3f}+-{m['ESS'][1]:.3f} "
        + " ".join(
            f"{k} {m[k][0] * 100:.1f}+-{m[k][1] * 100:.1f}"
...
def main():
    ...
    for sigma_label, sigma in COUPLINGS:
        ...
        for key, (c_target, lam, glob) in FAMILIES.items():
            ...
            exact_std_c = (
                ((ref_probs * (c_states - exact_mean_c) ** 2).sum()).sqrt().item()
            )
            ...
            if lam != 50.0:
                continue
            frame_leaf = (
                Path("eval") / "composition_sweep" / f"c{c_target:.4f}" / "samples.pt"
            )
            cond_dirs = [
                d
                for d in sorted(SOFT_RESULTS.glob(CONDITIONED.format(sc=suffix)))
