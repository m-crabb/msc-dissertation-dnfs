...
from discrete_flow_sampler.diagnostics.figure_style import (
    FIGSIZE_SINGLE, FONT_SIZE_ANNOTATION, GRID, MUTED, SAMPLER_HUE,
    SAVEFIG_DPI, parameter_ramp, seed_band, style_axes, use_house_style)
...
def load_family(run_dirs: list[Path]) -> dict:
    ...
    for run_dir in run_dirs:
        log = pd.read_csv(run_dir / "training_log.csv",
                          usecols=["step", "ess"]).dropna(subset=["ess"])
        if steps is None:
    ...
    final_stage_start = (lambda_curriculum["stages"][-1]["start_step"]
                         if lambda_curriculum else None)
    return {"steps": steps, "ess_fractions": per_seed,
...
def main() -> None:
    ...
    parser.add_argument("--fixed_runs", required=True, nargs="+", type=Path,
                        help="fixed lambda=50 run dirs (c=0.50, seeds 42-45)")
    parser.add_argument("--annealed_runs", required=True, nargs="+", type=Path,
                        help="lambda-annealed run dirs (c=0.50, seeds 42-45)")
    parser.add_argument("--out", type=Path,
                        default=Path("training_curves_soft.png"))
    ...
    ax.text(ramp_end / 1e3 * 0.5, 1.035, "$\\lambda$ anneal ramp",
            fontsize=FONT_SIZE_ANNOTATION, color=MUTED, ha="center")
    seed_band(ax, fixed["steps"] / 1e3, fixed["ess_fractions"],
              light_hue, "fixed $\\lambda=50$")
    seed_band(ax, annealed["steps"] / 1e3, annealed["ess_fractions"],
              dark_hue, "annealed")
    ...
    ax.legend(handles,
              ...
              fontsize=FONT_SIZE_ANNOTATION, frameon=False, loc="center right")
    style_axes(ax)
    fig.tight_layout()
    ...
    for name, family in (("fixed", fixed), ("annealed", annealed)):
        ...
        print(f"{name}: last in-training eval ESS fraction "
