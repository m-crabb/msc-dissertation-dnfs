...
def analyse(run_dir, t_value):
    ...
    out["r2_any_local_count"], residual = lookup_r_squared(S, keys_count, fit_mask)
    held = ~fit_mask
    ...
    out["nonlocal_share_explained_by_B"] = r_squared(
        residual,
        torch.stack([torch.ones_like(B_h), B_h, B_h**2, B_h * h_h, B_h**2 * h_h], 1),
    )
    return out
...
def main(argv=None):
    ...
    for r in rows:
        print(
            f"{r['run'][:58]:58s} {r['t']:4.1f}"
            + "".join(f" {r[c]:12.3f}" for c in COLUMNS)
            + f" {r['r2_channel_wt']:8.3f}"
        )
    if args.out:
