...
def main() -> None:
    ...
    print(
        f"=== mchammer canonical F(c) reference : D={args.D} sigma={args.sigma} "
    ...
    for r in rows:
        line = (
            f"{r['c_target']:>6.3f} {r['c_eff']:>6.3f} "
    ...
    pairs = sorted(
        {
            (round(min(c, 1 - c), 4), round(max(c, 1 - c), 4))
            for c in Fmap
    ...
    if pairs:
        ...
        for lo, hi in pairs:
            print(
                f"  F({lo:.3f})={Fmap[lo]:+.5f}  F({hi:.3f})={Fmap[hi]:+.5f}  "
