...
def reference_tv_floor(reference: torch.Tensor, pmf_of, seed: int = 0) -> float:
    ...
    for _ in range(N_FLOOR_BOOTSTRAP):
        blocks = torch.randint(
            0, n_blocks, (N_EVAL // FLOOR_BLOCK,), generator=generator
        )
...
def main() -> None:
    ...
    panels = {}
    for key, pmf_of, support in (
        ("energy", energy_pmf, support_energy),
        ("composition", composition_pmf, support_c),
    ):
        ref_pmf = pmf_of(reference, uniform)
        seed_pmfs = torch.stack(
            [pmf_of(run["samples"], run["weights"]) for run in seed_runs]
        )
        panels[key] = {
            "support": support,
            "ref": ref_pmf,
