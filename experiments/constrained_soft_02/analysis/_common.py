...
One definition rather than three clones: discovery has real logic (timestamped
vs bare run-dir forms, eval-completeness filter), and copies that drifted apart
would make two analyses disagree about which run they scored.
"""
...
def latest_run_dir(
    ...
    `eval_dir` selects which frozen eval qualifies a run as complete: "eval"
    (raw weights, every run) or "eval_ema" (the shadow-weight draw, present
    only on ema_decay > 0 cells), so an EMA analysis cannot score a raw draw.
    """
...
# reference (0.9950 at lambda=50), never the archived 0.976.
REVAMP_GRID = (
    0.25,
    0.3125,
    0.375,
