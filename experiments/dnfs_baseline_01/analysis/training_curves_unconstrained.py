...
def load_family(run_dirs: list[Path]) -> dict:
    ...
    final_stage_start = (curriculum["stages"][-1]["start_step"]
...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path,
                        default=Path("training_curves_unconstrained.png"))
    ...
    seed_band(ax, subcritical["steps"] / 1e3, subcritical["ess_fractions"],
              light_hue, f"$\\sigma = {subcritical['sigma']:g}$")
    seed_band(ax, critical["steps"] / 1e3, critical["ess_fractions"],
              dark_hue, "$\\sigma_c$ (curriculum)")
    ...
    ax.legend(handles,
              ...
              fontsize=FONT_SIZE_ANNOTATION, frameon=False, loc="lower right")
    style_axes(ax)
    fig.tight_layout()
