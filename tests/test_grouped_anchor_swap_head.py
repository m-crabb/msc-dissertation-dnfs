...
Contracts pinned here:
  ...
  * group chunking is numerically inert (incl. a ragged tail chunk);
...
@pytest.mark.parametrize("grouping", ["diagonal", "strided", "contiguous"])
def test_body_is_exactly_blind_to_every_masked_site(grouping):
...
def test_body_is_hollow_at_the_read_site():
    ...
    d, batch = 16, 3
    head = GroupedAnchorSwapHead(_backbone(d), n_groups=4, grouping="strided")
    x, t = _state_batch(d, batch), _time(batch)
...
def test_group_chunking_is_numerically_inert(chunk):
    """Chunking bounds the (n_groups*B, n_heads, d, 2d) readout buffer -- the
    memory term that makes mask_one infeasible at d=256 -- and must not change
    the result, including when the last chunk is ragged (8 groups / chunk 3)."""
    d, batch = 64, 2
    backbone = _backbone(d)
    x, t = _state_batch(d, batch), _time(batch)
    ...
    assert torch.equal(chunked(x, t), unchunked(x, t))
