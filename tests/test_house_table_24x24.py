...
def test_reference_bill_uses_this_rung_s_lattice():
    ...
    from experiments.constrained_hard_03.analysis.house_table_24x24 import L as fill_L
    ...
    assert (
        reference_trial_counts(provenance, lattice_side=24)
        == [(1000 + 2000) * 24 * 24] * 3
    )
...
def test_neural_cell_scores_profiles_on_the_fill_s_lattice(tmp_path):
    ...
    (run_dir / "eval" / "metrics.json").write_text(json.dumps({"ess_fraction": 1.0}))
    ...
    with pytest.raises(Exception):
        neural_cell(
            run_dir, target, reference, reference_energy, per_forward=1.0, n_euler=1
        )
    cell = neural_cell(
        run_dir,
        target,
...
@pytest.mark.skipif(
    not REFERENCE_DIR.is_dir(), reason="d576 sigma_c reference not generated"
)
...
def test_latex_body_is_single_coupling():
    ...
    from experiments.constrained_hard_03.analysis.house_table_24x24 import latex_table
