"""Tests for the triu-pair gather lever (opt-in, default off).

Every one-pass swap head assembles a pair context H that is label-symmetric
(H_ji := H_ij, the convention that makes index antisymmetry G[j,i] = -G[i,j]
an identity), then runs its per-pair nonlinear work -- the pair-readout MLP,
the masked-attention band, the hole-subtracted global MLP -- on all d^2
ordered pairs before mirroring. Half of that work is the mirror image of the
other half, and the diagonal is dead (forward reads H against
omega_{x_i} - omega_{x_j}, identically zero at i = j). The lever gathers the
d(d-1)/2 pairs with i < j before the nonlinear work and scatters the result
back, so the (B, d^2, F) activation slabs -- the heads' memory footprint at
d = 256 -- become (B, d(d-1)/2, F).

The correctness claim is a pure re-indexing claim, and is tested as one:

  * G matches the dense path to fp round-off (the per-pair maps are
    row-independent, so only GEMM shape changes);
  * H matches on i != j. The diagonal is not reproduced: it never reaches G,
    and recomputing it would reinstate d of the rows the lever drops;
  * index antisymmetry and the zero diagonal of G stay exact -- identities of
    the mirror, which a scatter writing the two triangles from different
    tensors would break;
  * the flag adds no parameter, no persistent buffer and no RNG draw, so a
    flag-off head is byte-identical to every checkpoint on disk;
  * blindness is pinned in each head's own suite, where the blindness probes
    are parametrised over the flag.

Heads with no symmetric per-pair slab to drop, so not covered (audited
2026-08-26): two_hole_patch (H_ij = P_ij - P_ji needs both orderings),
mask_one / grouped_anchor (H_ij = H^{group(i)}[:, j, :] is not symmetric, and
the cost is the per-site masked body passes), and the exact-field channel
(sigma * Delta_ij is elementwise arithmetic at width 1).
"""
...
# Re-indexing bar: the per-pair maps are row-independent, so the only source of
# drift is GEMM/reduction shape (a (B, d^2, F) matmul vs a (B, P, F) one) --
# fp round-off on quantities of order 1, two orders tighter than the suite's
# blindness atol.
GATHER_ATOL = 1e-6
...
def _pair(case, gather_on=True):
    ...
    draw from the flag => identical parameters, so any output difference is the
    lever."""
...
def test_gathered_pair_context_matches_dense_off_diagonal(case):
    """H agrees wherever it is read. The diagonal is exempt: the token
    difference is zero there, so H_ii never reaches G, and the gathered path
    leaves it at zero rather than paying for d dead rows."""
    dense, gathered = _pair(case)
    x, t = _state(), torch.rand(2)
...
def test_gathered_context_is_exactly_symmetric(case):
    ...
    approximate, which the swap CTMC's reverse rate leans on."""
...
def test_gathered_gradients_reach_every_head_parameter(case):
    ...
    index_put that dropped its input from the graph would train silently wrong,
    which no forward-value test would catch."""
    _, gathered = _pair(case)
    gathered.train()
...
def test_band_summaries_pair_form_matches_dense_triangle(build):
    """The band's gathered form moves the (B, d^2, n_terms) attention scores --
    the masked-attention head's largest tensor at d = 256 -- onto the i < j
    pairs. Both heads define the band only for i < j, so the pair form must
    equal the dense upper triangle where it is defined."""
...
def test_config_flag_is_a_no_op_for_the_heads_without_a_symmetric_slab():
    """Cells that do not read the flag build byte-identically with it set.
    two_hole_patch needs both orderings of every pair (H_ij = P_ij - P_ji) and
    mask_one's H is not label-symmetric, so neither has a triangle to drop."""
    from dataclasses import replace
