...
The d400 swap step is bandwidth-bound, not FLOP-bound: profiled at 36% GEMM,
with those GEMMs at 8.0 FLOP/byte against the A100's fp32 ridge point of 10.1.
Levers that reduce arithmetic do almost nothing (TF32 measured -10% time,
-0% memory); levers that reduce bytes do a lot. Measured d400 R=3, batch 512,
compiled, A100-80GB:
...
Two things must not move:
  ...
  * the importance weights. bf16 reaching the target's swap log-ratio would
    make this an estimator change, not a speed lever. It does not: the closed
    form is t*sigma*[2(x_j - x_i)(h_i - h_j) - 2(x_j - x_i)^2 A_ij] with
    h = x @ A, and h is a sum over four torus neighbours, so it lives in
    {-4, -2, 0, 2, 4} at every lattice size -- integers bf16 represents
    exactly. Measured drift 0.000e+00 at D=16 and D=20.

  * the frozen eval. The flag is scoped to the loss update, so a cell that
    trains in bf16 is still evaluated in fp32.
...
# Both couplings: the floor pair settled cost (4/4 matched seeds, -21% to
# -27% end-to-end); the sigma_c pair settles quality, which the floor could
# not because every cell there sat on the sampling ceiling.
...
def test_swap_log_ratio_is_bit_identical_under_bf16_autocast():
    """If this fails, the flag stops being a speed lever and becomes a change
...
def test_kawasaki_field_is_integer_valued_at_every_lattice_size():
    """Why the gate above passes, pinned separately so a change to the
    adjacency convention cannot silently invalidate it: h = x @ A is a sum
    over four torus neighbours, so it is a small integer regardless of d,
    and small integers are exact in bf16."""
    for D in (8, 16, 20):
        target = _target(D)
...
def test_train_autocast_bf16_defaults_off_everywhere():
    ...
    Only the `_w4bf16` d400 probe cells and the `_w5bf16` d576 cells
    (bf16-only at d400+) may carry it."""
...
def test_bf16_probe_cells_are_their_fp32_twins_plus_the_flag():
    """A `_w4bf16` cell must differ from its `_w4` sibling in
    `train_autocast_bf16` and in nothing else, so a bf16-vs-fp32 gap is
    chargeable to the precision. Covers both couplings: the twin-ness has to
    hold at sigma_c, the coupling where the columns can separate."""
...
def test_frozen_eval_stays_fp32_on_a_bf16_trained_cell():
    """bf16 is scoped to the loss update, so the end-of-run eval config of a
    bf16-trained cell is identical to its fp32 twin's."""
    from experiments.constrained_hard_03.configs import CONFIGS
...
def test_training_autocast_is_wired_into_the_swap_trainer():
    """The flag must reach the loss update; a config field no code reads
