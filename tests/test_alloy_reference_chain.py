...
STATES = torch.tensor(list(itertools.product([-1.0, 1.0], repeat=16)), dtype=torch.float64)
...
def _exact(temperature_K, composition=None):
    ...
    mask = N_AU == round(composition * 16) if composition is not None else torch.ones_like(N_AU, dtype=torch.bool)
...
def test_free_chain_matches_enumeration_at_1200K():
    ...
    states, _ = run_chains(target, n_chains=64, burn_in_sweeps=200, n_records=100,
                           thin_sweeps=5, seed=0, canonical=False)
...
def test_kawasaki_chain_stays_on_the_slice_and_matches_enumeration():
    ...
    states, _ = run_chains(target, n_chains=64, burn_in_sweeps=200, n_records=100,
                           thin_sweeps=5, seed=1, canonical=True)
    ...
    assert abs(float(obs["energy_per_site"].mean()) - exact["energy_per_site"]) < 4 * se + 1e-4


def test_beta_ladder_recovers_the_exact_canonical_free_energy():
    result = beta_ladder_free_energy(
        SPEC, 800.0, composition=0.25, canonical=True, n_grid=9, n_chains=64,
        burn_in_sweeps=100, n_records=40, thin_sweeps=3, seed=2)
