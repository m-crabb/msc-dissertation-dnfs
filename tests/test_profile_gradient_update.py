...
@pytest.mark.parametrize("mode,expected_draws", [
    ("gfn_update", 1), ("gfn_train_step", 2),
])
def test_gfn_update_prepares_one_batch_outside_timing(monkeypatch, mode, expected_draws):
    ...
    monkeypatch.setattr(run_gfn, "build_target_and_policy", lambda *args: (None, policy))
    monkeypatch.setattr(run_gfn, "build_optimiser", lambda *args:
                        torch.optim.AdamW([parameter], lr=0.01))
    ...
    args = SimpleNamespace(d=64, batch=2, gfn_objective="tb", mode=mode,
                           compile=False, repeats=2, profile=False, warmup=3)
...
def test_timing_discards_warmup_and_resets_gpu_peak(monkeypatch):
    ...
    times = profile_swap._timed(lambda: calls.append("update"), 2,
                               torch.device("cuda"), warmup=3)
