...
The swap trainer has had this since a Modal GPU recall; the flip trainer did
not, and a matched-base family paid for it — eight runs killed at ~94% of a
50k-step budget with nothing on disk but a weights-only `latest.pt`.

Properties pinned:

1. Bit-exact continuation: a run interrupted at an outer-cycle boundary and
   resumed reproduces an uninterrupted run bit-for-bit on CPU fp32. The
   checkpoint therefore carries model + optimiser (AdamW moments), the step
   counter, the torch RNG states, and the replay buffer, which with
   `replay_buffer_cycles > 1` holds past outer trajectories that cannot be
   reconstructed from anything else.
2. All four replay lists travel. The amortised path retains per-state
   composition and per-state c_t baselines alongside the states and time
   indices; dropping either still trains, on states paired with the wrong
   baseline, so it gets its own test.
3. Curriculum fast-forward. Stage indices are derived from the step counter,
   not stored: the amortised test crosses a σ boundary before the interruption
   and λ and composition boundaries after it.
4. Arming resume does not perturb an uninterrupted run — writing a checkpoint
   must not touch the RNG stream or anything else the trajectory depends on.
5. Orphan log rows. A dead attempt flushes a CSV row every inner step but
   checkpoints only every `resume_every_outer` cycles; resume drops the rows
   past the checkpoint so every step appears exactly once.
"""
...
def _die_after(n_checkpoints: int):
    ...
    Raising from the hook rather than truncating `n_steps` keeps the twin a
    faithful preemption: resume.pt is already on disk (the hook fires after
    the write), the CSV holds whatever was flushed, and none of the
    end-of-run artefacts exist.
    """
    calls = {"n": 0}
...
def test_arming_resume_does_not_perturb_an_uninterrupted_run(tmp_path):
    ...
    move the trajectory: a run that checkpoints every outer cycle and one that
    never checkpoints at all agree step for step, which is what keeps archived
    flip runs comparable to runs from the new trainer."""
